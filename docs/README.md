# Documentation

## Quick Start Guides

- [Quick Start: Data Configuration](quick-start-data-configuration.md) - Redirecting data directories
- [Quick Start: Offline Bundle Creation](quick-start-offline-bundling.md) - Creating offline packages

## Deployment Guides

- [Container Guide](container-guide.md) - Docker/Podman on any OS
- [Python Guide](python-guide.md) - Direct Python execution on any OS
- [Scheduled Execution](scheduled-execution.md) - Cron, systemd, Task Scheduler
- [Offline Systems](offline-systems.md) - Air-gapped deployments

## Configuration

- [Configuration Reference](configuration-reference.md)
- [Example Configs](example_configs/README.md)

## Advanced

- [Building Images](building-images.md) - Local builds and CI/CD setup